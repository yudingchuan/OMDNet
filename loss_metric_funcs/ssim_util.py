...
def _ssim(img1, img2, window, window_size, channel, weight=None):
    ...
    if weight is None:
        ...
        crop = window_size // 2
        weight = weight[ :, crop:-crop, crop:-crop]
        return (ssim_map * weight).sum() / weight.sum().clamp_min(1) / img1.shape[0]


def ssim_calc(img1, img2, weight, window_size=11):
    ...
    return _ssim(img1, img2, window, window_size, channel, weight)
